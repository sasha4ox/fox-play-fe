...
export async function generateMetadata({ params }) {
  ...
  const t = await getTranslations({ locale, namespace: 'PaymentModel' });
  return { title: t('title') };
}

export default async function PaymentModelPage({ params }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('PaymentModel');
  const base = `/${locale}`;
