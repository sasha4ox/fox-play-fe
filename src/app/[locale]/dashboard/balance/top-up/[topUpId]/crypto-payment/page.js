...
import {
  getBalanceTopUpCryptoPayment,
  markBalanceTopUpCryptoSent,
  extendBalanceTopUpCryptoDeadline,
} from '@/lib/api';
...
export default function BalanceTopUpCryptoPaymentPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getBalanceTopUpCryptoPayment(topUpId, token)
      .then(setData)
      .catch((err) => {
  ...
  const handleExtendDeadline = () => {
    ...
    setExtending(true);
    setError(null);
    extendBalanceTopUpCryptoDeadline(topUpId, token)
      .then(() => getBalanceTopUpCryptoPayment(topUpId, token))
  ...
  const handleConfirmSentClick = () => {
    ...
    setSending(true);
    setError(null);
    markBalanceTopUpCryptoSent(topUpId, token)
      .then(() => getBalanceTopUpCryptoPayment(topUpId, token))
      .then((updated) => {
        setData(updated);
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 4 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mt: 3 }}>
        ...
        <Button component={Link} href={balanceHref} variant="outlined" sx={{ minHeight: 48 }}>
